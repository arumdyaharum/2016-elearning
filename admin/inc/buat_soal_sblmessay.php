		    ...
		    <div class="panel-body">
	            ...
	            if(@$_POST['simpan']) {
					...
					if($nama_gambar[0] == ''){
						...
						if( $type[0] == 'audio/mp3' || $type[0] == 'audio/x-mp3' || $type[0] == 'audio/mpeg' || $type[0] == 'audio/x-mpeg' || $type[0] == 'audio/mpeg3' || $type[0] == 'audio/x-mpeg-3' ) //If this filetype is actually permitted
						{
							//echo "sip";
					...
					if($filetype == "audio"){
						...
						move_uploaded_file($sumber[1], $target_soal.$id.'_'.$nama_gambar[1]);
						for($i=2;$i<7;$i++){move_uploaded_file($sumber[$i], $target_jawab.$id.'_'.$nama_gambar[$i]);}
						mysqli_query($conn, "INSERT INTO tb_soal_pilgan VALUES('', '$id', '$pertanyaan', '".$id."_".$nama_gambar[0]."', '".$id."_".$nama_gambar[1]."', '$pilA', '".$id."_".$nama_gambar[2]."', '$pilB', '".$id."_".$nama_gambar[3]."', '$pilC', '".$id."_".$nama_gambar[4]."', '$pilD', '".$id."_".$nama_gambar[5]."', '$pilE', '".$id."_".$nama_gambar[6]."', '$kunci', now())") or die ($db->error); //echo "sip";
						echo "<br><br>".$id.' '.$pertanyaan.' '.$nama_gambar['0'].' '.$nama_gambar['1'].' '.$pilA.' '.$nama_gambar['2'].' '.$pilB.' '.$nama_gambar['3'].' '.$pilC.' '.$nama_gambar['4'].' '.$pilD.' '.$nama_gambar['5'].' '.$pilE.' '.$nama_gambar['6'].' '.$kunci;
						echo '<script>window.location="?page=quiz&action=daftarsoal&hal=pilgan&id='.$id.'"</script>';
					} else {
		...
		<div class="panel panel-default">
		    ...
		    <div class="panel-body">
	            ...
	            if(@$_POST['simpan']) {
                    ...
                    move_uploaded_file($sumber, $target.$nama_gambar);
                    mysqli_query($conn, "INSERT INTO tb_soal_essay VALUES('', '$id', '$pertanyaan', '$nama_gambar', '$jawaban', now())") or die ($db->error);          
