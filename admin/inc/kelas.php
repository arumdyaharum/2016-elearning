...
<?php
$id = @$_GET['id'];
$sql_per_id = mysqli_query($conn, "SELECT * FROM tb_kelas WHERE id_kelas = '$id'") or die ($db->error);
...
if(@$_SESSION[admin]) {
    ...
    if(@$_GET['action'] == '') { ?>
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading"><a href="?page=kelas&action=tambah" class="btn btn-primary btn-sm">Tambah Kelas</a> &nbsp;</div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            while($data_kelas = mysqli_fetch_array($sql_kelas)) { ?>
                                <tr>
                                    <td><?php echo $no++; ?></td>
                                    <td><?php echo $data_kelas['ruang'].' - '.$data_kelas['nama_kelas']; ?></td>
                                    <td align="center" width="200px">
                                        ...
                                        <a href="?page=siswa&IDkelas=<?php echo $data_kelas['id_kelas']; ?>&kelas=<?php echo $data_kelas['nama_kelas']; ?>" class="badge">Lihat Mahasiswa</a>
                                    </td>
                                </tr>
                            <?php
                            } ?>
    ...
    <?php
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    if(@$_POST['simpan']) {
                        ...
                        mysqli_query($conn, "INSERT INTO tb_kelas(nama_kelas, ruang) VALUES('$nama_kelas', '$ruang')") or die ($db->error);
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    if(@$_POST['simpan']) {
                        $nama_kelas = @mysqli_real_escape_string($conn, $_POST['nama_kelas']);
                        $ruang = @mysqli_real_escape_string($conn, $_POST['ruang']);
                        mysqli_query($conn, "UPDATE tb_kelas SET nama_kelas = '$nama_kelas', ruang = '$ruang' WHERE id_kelas = '$id'") or die ($db->error);
                        echo "<script>window.location='?page=kelas';</script>";
                    }
                    ?>
                </div>
        ...
        mysqli_query($conn, "DELETE FROM tb_kelas WHERE id_kelas = '$id'") or die ($db->error);  
        echo "<script>window.location='?page=kelas';</script>"; 
    }
    echo "</div>";
    ...
    if(@$_GET['action'] == '') { ?>
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-heading">Kelas yang Anda Ajar &nbsp; <a href="?page=kelas&action=tambah" class="btn btn-primary btn-sm">Tambah Kelas</a></div>
                    <div class="panel-body">
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover">
                                <thead>
                                ...
                                <tbody>
                                <?php
                                $sql_ajar = mysqli_query($conn, "SELECT * FROM tb_kelas_ajar JOIN tb_kelas ON tb_kelas_ajar.id_kelas = tb_kelas.id_kelas WHERE tb_kelas_ajar.id_pengajar = '$_SESSION[pengajar]'") or die ($db->error);
                                if(mysqli_num_rows($sql_ajar) > 0) {
                                    while($data_ajar = mysqli_fetch_array($sql_ajar)) { ?>
                                        <tr>
                                            <td><?php echo $no++; ?></td>
                                            <td><?php echo $data_ajar['ruang'].' - '.$data_ajar['nama_kelas']; ?></td>
                                            ...
                                            <td align="center" width="240px">
                                                ...
                                                <a href="?page=siswa&IDkelas=<?php echo $data_ajar['id_kelas']; ?>&kelas=<?php echo $data_ajar['nama_kelas']; ?>" class="btn btn-default btn-xs">Lihat Mahasiswa</a>
                                            </td>
                                        </tr>
                                    <?php
                                    }
    ...
    <?php
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        if(@$_POST['simpan']) {
                            ...
                            mysqli_query($conn, "INSERT INTO tb_kelas_ajar VALUES('', '$kelas', '$pengajar', '$ket')") or die ($db->error);
                            echo "<script>window.location='?page=kelas';</script>";
                        }
                        ?>
                    </div>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                    <?php
                    $sql_id_ajar = mysqli_query($conn, "SELECT * FROM tb_kelas_ajar JOIN tb_kelas ON tb_kelas_ajar.id_kelas = tb_kelas.id_kelas WHERE tb_kelas_ajar.id = '$id'") or die ($db->error);
                    ...
                    ?>
                        ...
                        if(@$_POST['simpan']) {
                            $kelas = @mysqli_real_escape_string($conn, $_POST['kelas']);
                            $ket = @mysqli_real_escape_string($conn, $_POST['ket']);
                            mysqli_query($conn, "UPDATE tb_kelas_ajar SET id_kelas = '$kelas', id_pengajar = '$_SESSION[pengajar]', keterangan = '$ket' WHERE id = '$id'") or die ($db->error);
                            echo "<script>window.location='?page=kelas';</script>";
                        }
                        ?>
                    </div>
        ...
        mysqli_query($conn, "DELETE FROM tb_kelas_ajar WHERE id = '$id'") or die ($db->error);
        echo "<script>window.location='?page=kelas';</script>";
    }
} ?>