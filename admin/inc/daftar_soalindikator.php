								...
								<table class="table">
									...
									<tbody>
										<tr>
											<td>Indikator</td>
											<td>:</td>
											<td><?php echo $data_pilgan['indikator']; ?></td>
										</tr>
										<tr>
											<td>Gambar</td>
											<td>:</td>
										...
										<tr>
											<td>Pembahasan</td>
											<td>:</td>
											<td><?php echo $data_pilgan['pembahasan']; ?></td>
										</tr>
										<tr>
											<td>Kunci</td>
											<td>:</td>
...
<?php
		...
		<div class="panel panel-default">
			...
			?>
				<form method="post" enctype="multipart/form-data">
					...
					<div class="col-md-10">
						<div class="form-group">
							<select name="indikator" class="form-control" required>
                                    ...
                                    <?php
									$sql_indikator = mysqli_query($conn, "SELECT * FROM tb_indikator WHERE tb_indikator.id_pengajar='$_SESSION[pengajar]'") or die ($db->error);
                                    while($data_indikator = mysqli_fetch_array($sql_indikator)) {
                                        echo '<option value="'.$data_indikator['id_indikator'].'">'.$data_indikator['indikator'].'</option>';
                                    }
                                    ?>
                                </select>
						</div>
					...
					<div class="col-md-10">
						<div class="form-group">
							<textarea name="pembahasan" class="form-control" rows="1" required><?php echo $data_pilgan_id['pembahasan']; ?></textarea>
						</div>
					</div>
					<div class="col-md-2">
				...
				if(@$_POST['simpan']) {
					...
					if(@$_GET['gbr'] == 'del') {
						if($nama_gambar == '') {
							mysqli_query($conn, "UPDATE tb_soal_pilgan SET pertanyaan = '$pertanyaan', indikator = '$indikator', gambar = '', pil_a = '$pilA', pil_b = '$pilB', pil_c = '$pilC', pil_d = '$pilD', pil_e = '$pilE', pembahasan = '$pembahasan', kunci = '$kunci' WHERE id_pilgan = '$idsoal'") or die ($db->error);
							// echo "gambar benar2 dihapus";
						} else {
							move_uploaded_file($sumber, $target.$nama_gambar);
							mysqli_query($conn, "UPDATE tb_soal_pilgan SET pertanyaan = '$pertanyaan', indikator = '$indikator', gambar = '$nama_gambar', pil_a = '$pilA', pil_b = '$pilB', pil_c = '$pilC', pil_d = '$pilD', pil_e = '$pilE', pembahasan = '$pembahasan', kunci = '$kunci' WHERE id_pilgan = '$idsoal'") or die ($db->error);
							// echo "gambar dihapus dan diupload baru";
						}
					} else {
						if($nama_gambar == '') {
							mysqli_query($conn, "UPDATE tb_soal_pilgan SET pertanyaan = '$pertanyaan', indikator = '$indikator', pil_a = '$pilA', pil_b = '$pilB', pil_c = '$pilC', pil_d = '$pilD', pil_e = '$pilE', pembahasan = '$pembahasan', kunci = '$kunci' WHERE id_pilgan = '$idsoal'") or die ($db->error);          
	...
	<div class="row">
		<div class="panel panel-default">
			...
			<div class="panel-body">
			<?php
			//$sql_essay_id = mysql_query("SELECT * FROM tb_soal_essay WHERE id_essay = '$idsoal'") or die ($db->error);
			//$data_essay_id = mysql_fetch_array($sql_essay_id);
			?>
				<form method="post" enctype="multipart/form-data">
					<div class="col-md-2">
						<label>Pertanyaan No. [ <?php echo $ke; ?> ]</label>
	            ...
	            if(@$_POST['simpan']) {
                    ...
                    if(@$_GET['gbr'] == 'del') {
						if($nama_gambar == '') {
							mysqli_query($conn, "UPDATE tb_soal_essay SET pertanyaan = '$pertanyaan', indikator = '$indikator', gambar = '' WHERE id_essay = '$idsoal'") or die ($db->error);
							// echo "gambar benar2 dihapus";
						} else {
							move_uploaded_file($sumber, $target.$nama_gambar);
							mysqli_query($conn, "UPDATE tb_soal_essay SET pertanyaan = '$pertanyaan', indikator = '$indikator', gambar = '$nama_gambar' WHERE id_essay = '$idsoal'") or die ($db->error);
							// echo "gambar dihapus dan diupload baru";
						}
					} else {
						if($nama_gambar == '') {
							...
							move_uploaded_file($sumber, $target.$nama_gambar);
							mysqli_query($conn, "UPDATE tb_soal_essay SET pertanyaan = '$pertanyaan', gambar = '$nama_gambar' WHERE id_essay = '$idsoal'") or die ($db->error);
							// echo "gambar diperbarui dari sebelumnya";
						}
					}        
