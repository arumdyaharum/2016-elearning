    ...
    <div class="row">
        <div class="col-md-12">
            <div class="panel panel-default">
                ...
                <?php
                if(@$_GET['IDkelas'] == '') {
                    echo 'Data Siswa yang Aktif &nbsp; <a href="./laporan/cetak.php?data=siswa&id=all&filename=elearning_siswaterdaftar_aktif_'.date("dmY").'.pdf" target="_blank" class="btn btn-default btn-xs">Cetak Data Siswa</a>';
                } else if(@$_GET['IDkelas'] != '') {
                    echo "Data Siswa Per Kelas ".@$_GET['kelas']." yang Aktif &nbsp; <a href='?page=kelas' class='btn btn-warning btn-sm'>Kembali</a> &nbsp; <a href='./laporan/cetak.php?data=siswa&id=".@$_GET['IDkelas']."&kelas=".@$_GET['kelas']."&filename=elearning_siswa_kelas_".@$_GET['kelas'].".pdf' target='_blank' class='btn btn-default btn-xs'>Cetak Data Siswa</a>";
                } ?>
                ...
                <div class="panel-body">
                	<div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover" id="datasiswa">
                            ...
                            if(@$_GET['IDkelas'] == '') {
                                $sql_siswa = mysqli_query($conn, "SELECT * FROM tb_siswa JOIN tb_kelas ON tb_siswa.id_kelas = tb_kelas.id_kelas WHERE tb_siswa.status = 'aktif'") or die ($db->error);
                            } else if(@$_GET['IDkelas'] != '') {
                                $sql_siswa = mysqli_query($conn, "SELECT * FROM tb_siswa JOIN tb_kelas ON tb_siswa.id_kelas = tb_kelas.id_kelas WHERE tb_siswa.status = 'aktif' AND tb_siswa.id_kelas = '$_GET[IDkelas]' ORDER BY tb_siswa.nis") or die ($db->error);
                            }
...
<?php
    ...
    mysqli_query($conn, "UPDATE tb_siswa SET status = 'tidak aktif' WHERE id_siswa = '$id'") or die ($db->error);
    ...
    $sql_siswa_per_id = mysqli_query($conn, "SELECT * FROM tb_siswa JOIN tb_kelas ON tb_siswa.id_kelas = tb_kelas.id_kelas WHERE id_siswa = '$_GET[IDsiswa]'") or die ($db->error);
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <div class="table-responsive">
                        <table width="100%">
                            <tr>
                                <td align="right" width="46%"><b>NISN</b></td>
                                <td align="center">:</td>
