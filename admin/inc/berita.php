	...
	<div class="row">
		<div class="col-md-12">
	        <div class="panel panel-default">
	            <div class="panel-heading">Data Berita &nbsp; <a href="?page=berita&action=tambah" class="btn btn-primary btn-xs">Tambah Berita</a> &nbsp; <a href="./laporan/cetak.php?data=berita" target="_blank" class="btn btn-default btn-xs">Cetak</a></div>
	            <div class="panel-body">
	                <div class="table-responsive">
	                    <table class="table table-striped table-bordered table-hover" id="databerita">
	                        ...
	                        $no = 1;
	                        if(@$_SESSION['admin']) {
		                        $sql_berita = mysqli_query($conn, "SELECT * FROM tb_berita") or die($db->error);
	                        } else if(@$_SESSION['pengajar']) {
	                        	$sql_berita = mysqli_query($conn, "SELECT * FROM tb_berita WHERE penerbit = '$_SESSION[pengajar]'") or die($db->error);
	                        }
									...
									<tr>
										...
										<td>
											...
											if($data_berita['penerbit'] == 'admin') {
												...
												$sql_pengajar = mysqli_query($conn, "SELECT * FROM tb_pengajar WHERE id_pengajar = '$data_berita[penerbit]'") or die($db->error);
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    if(@$_POST['simpan']) {
                    	...
                    	if(@$_SESSION[admin]) {
                            mysqli_query($conn, "INSERT INTO tb_berita VALUES('', '$judul', '$isi', now(), 'admin', '$status')") or die ($db->error);           
                        } else if(@$_SESSION[pengajar]) {
                        	mysqli_query($conn, "INSERT INTO tb_berita VALUES('', '$judul', '$isi', now(), '$_SESSION[pengajar]', '$status')") or die ($db->error);
                        }
	...
	<div class="row">
		<div class="col-md-6">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                <?php
                $sql_beritaID = mysqli_query($conn, "SELECT * FROM tb_berita WHERE id_berita = '$_GET[id]'") or die($db->error);
                $data_beritaID = mysqli_fetch_array($sql_beritaID); ?>
                    ...
                    if(@$_POST['simpan']) {
                    	...
                    	$isi = @mysqli_real_escape_string($conn, $_POST['isi']);
                    	$status = @mysqli_real_escape_string($conn, $_POST['status']);
                        mysqli_query($conn, "UPDATE tb_berita SET judul = '$judul', isi = '$isi', status = '$status' WHERE id_berita = '$_GET[id]'") or die ($db->error);           
                    	echo '<script>window.location="?page=berita";</script>';
                    } ?>
                </div>
            </div>
...
<?php
	...
	mysqli_query($conn, "DELETE FROM tb_berita WHERE id_berita = '$_GET[id]'") or die($db->error);
	echo '<script>window.location="?page=berita";</script>';
} ?>