            ...
            <div class="panel-body">
                <div class="table-responsive">
                	<table class="table table-striped table-bordered table-hover" id='table'>
                		...
                		<?php
                		$no = 1;
                		$sql_cek_nilai_pilgan = mysql_query("SELECT * FROM tb_nilai_pilgan JOIN tb_topik_quiz ON tb_nilai_pilgan.id_tq = tb_topik_quiz.id_tq JOIN tb_mapel ON tb_topik_quiz.id_mapel = tb_mapel.id WHERE id_siswa = '$_SESSION[siswa]'") or die ($db->error);
                		if(mysql_num_rows($sql_cek_nilai_pilgan) > 0) {
                			while($data_nilai_pilgan = mysql_fetch_array($sql_cek_nilai_pilgan)) { ?>
                				<tr>
	                				<td><?php echo $no++; ?></td>
                					...
                					<?php
                					//$sql_cek_jawaban = mysql_query("SELECT * FROM tb_jawaban WHERE id_tq = '$data_nilai_pilgan[id_tq]' AND id_siswa = '$_SESSION[siswa]'") or die ($db->error);
                					//$data_jawaban = mysql_fetch_array($sql_cek_jawaban);
                					//if(mysql_num_rows($sql_cek_jawaban) > 0) {
                						//$sql_cek_nilai_essay = mysql_query("SELECT * FROM tb_nilai_essay WHERE id_tq = '$data_nilai_pilgan[id_tq]' AND id_siswa = '$_SESSION[siswa]'") or die ($db->error);
                						//$data_nilai_essay = mysql_fetch_array($sql_cek_nilai_essay);
                						//if(mysql_num_rows($sql_cek_nilai_essay) > 0) { 
