...
$key = $_POST["nomorsoal"];
$tampil = $_POST["tampil"];
$pilgan = $_POST["jmlpilgan"];

if($tampil > $pilgan){
	$query = mysql_query('select ragu,jawaban from tb_essay_jawaban where id_tq = "'.$id_tq.'" && id_soal = "'.$key.'" && id_siswa = "'.$siswa.'"');
	$cek = mysql_num_rows($query);
	$array = mysql_fetch_array($query);

	if($cek > 0){
		if($array['ragu'] == '1'){
		mysql_query('update tb_essay_jawaban set ragu = "0" where id_tq = "'.$id_tq.'" && id_soal = "'.$key.'" && id_siswa = "'.$siswa.'"');
		echo $tampil.' 0 '.$array['jawaban'];
		} else {
		mysql_query('update tb_essay_jawaban set ragu = "1" where id_tq = "'.$id_tq.'" && id_soal = "'.$key.'" && id_siswa = "'.$siswa.'"');
		echo $tampil.' 1 '.$array['jawaban'];
		}
	}else{
		mysql_query('insert into tb_essay_jawaban values ("", "'.$id_tq.'", "'.$key.'", "'.$siswa.'", "1", "")');
		echo $tampil.' 1 '.$array['jawaban'];
	}
} else {
